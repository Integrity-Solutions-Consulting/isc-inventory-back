...
public interface WarrantTypeRepository extends JpaRepository<WarrantTypeEntity, Integer>{
	...
	@Modifying
	@Transactional
	@Query("UPDATE WarrantTypeEntity u SET u.status=false, u.modificationDate = CURRENT_TIMESTAMP WHERE u.id = :id AND u.status = true")
	int inactive(@Param("id") Integer id);
	
    @Modifying
    @Transactional
    @Query("UPDATE WarrantTypeEntity u SET u.status = true, u.modificationDate = CURRENT_TIMESTAMP WHERE u.id = :id AND u.status = false")
