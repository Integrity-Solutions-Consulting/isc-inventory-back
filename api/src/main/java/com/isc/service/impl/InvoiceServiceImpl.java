...
public class InvoiceServiceImpl implements InvoiceService{
	...
	public ResponseDto<List<InvoiceDetailResponseDTO>> getAllDetails() {
		List<InvoiceDetailResponseDTO> response = invoiceRepository.findAll().stream().map(InvoiceMapper::toInvoiceDetailDto)
	...
	public ResponseDto<List<InvoiceResponseDTO>> getSimpleList() {
		List<InvoiceResponseDTO> response = invoiceRepository.findAllByStatusTrue().stream().map(InvoiceMapper::toInvoiceDto)
				.collect(Collectors.toList());
		MetadataResponseDto metadata = new MetadataResponseDto(HttpStatus.OK, "Facturas listadas correctamente");
		return new ResponseDto<>(response, metadata);
	}
	...
	public ResponseDto<InvoiceDetailResponseDTO> save(InvoiceRequestDTO request) {
		InvoiceDetailEntity invoiceDetail = invoiceDetailRepository.findById(request.getInvoiceDetail())
				.orElseThrow(() -> new RuntimeException("Detalle de factura no encontrada"));
		SupplierEntity supplier = supplierRepository.findById(request.getSupplier())
				.orElseThrow(() -> new RuntimeException("Proveedor no encontrado"));
		InvoiceEntity entity = new InvoiceEntity();
		entity.setInvoiceDetail(invoiceDetail);
	...
	public ResponseDto<InvoiceDetailResponseDTO> update(InvoiceRequestDTO request, Integer id) {
		...
		if(entity.getInvoiceDetail().getId()!=request.getInvoiceDetail()) {
			InvoiceDetailEntity invoiceDetail = invoiceDetailRepository.findById(request.getInvoiceDetail())
		            .orElseThrow(() -> new RuntimeException("Detalle de factura no encontrada"));
		    entity.setInvoiceDetail(invoiceDetail);
		}
		if(entity.getSupplier().getId()!=request.getSupplier()) {
			SupplierEntity supplier = supplierRepository.findById(request.getSupplier())
